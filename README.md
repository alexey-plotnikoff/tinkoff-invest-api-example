...
Поддержка трейдинга списком ценных бумаг должна заниматься стратегия.
...
Данная стратегия ничего не делает :). Если вдруг Вам нужно просто запустить приложение без торгов.
...
3. Переносит Вас в прошлой
4. `IntervalStrategy` инициализируется в прошлом
5. Накидывает все свечи от момента в прошлом до настоящего сегодня(CANDLE_INTERVAL_1_MIN)
...
3. Для получения различной информации
